...
import { CourseWithId, Lesson, LessonWithId, ModuleWithId, Quiz } from '@/types';
...
const AddLesson = ({ courses, modules,lesson }: CoursesProps) => {
    const { flash } = usePage<CustomPageProps>().props;
    const [selectedCourseId, setSelectedCourseId] = useState(String(lesson.course_id || ''));
    ...
    const submit: FormEventHandler = (e) => {
        ...
        post(route('update.lesson',{id:lesson.id}), {
            onFinish: () => {
                reset('module_id', 'title', 'video_url', 'duration');
                setSelectedCourseId('');
            },
        });
    };
    ...
    return (
        <AppLayout>
            <PageBreadcrumb pageTitle="Edit Lesson" subTitle="All Lesson" link="/all/lesson" />
                    ...
                    <form onSubmit={submit} encType="multipart/form-data">
                        ...
                        <div>
                            <Button
                                ...
                                disabled={processing}
                            >
                                {processing && <LoaderCircle className="h-4 w-4 animate-spin" />}Update Lesson
                            </Button>
                        </div>
                    </form>
                </div>
